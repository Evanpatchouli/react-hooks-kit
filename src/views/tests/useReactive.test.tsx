...
describe("useReactive", () => {
  ...
  it("should react to state changes when deep is false", () => {
    const { result } = renderHook(() => useReactive({ test: { inner: "test" } }, false));
  ...
  it("should react to state changes when deep is true", () => {
    const { result } = renderHook(() => useReactive({ test: { inner: "test" } }, true));
  ...
  it("should trigger callback on state change", () => {
    ...
    const { result } = renderHook(() => useReactive({ test: "test" }, callback));
  ...
  it("should trigger multiple callbacks on state change", () => {
    ...
    const { result } = renderHook(() => useReactive({ test: "test" }, callback1, callback2));
  ...
  it("should handle nested array state", () => {
    ...
    // act(() => {
    //   result.current.test.push(4);
    // });

    expect(result.current.test).toEqual([1, 2, 3]);
  });

  it("should handle function state", () => {
    const { result } = renderHook(() => useReactive({ test: () => "test" as string }));
  ...
  it("should handle nested function state", () => {
    const { result } = renderHook(() => useReactive({ test: { inner: () => "test" as string } }));
